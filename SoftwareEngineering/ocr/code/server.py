...
class JSONHandler(http.server.BaseHTTPRequestHandler):
    def do_POST(s):
        ...
        if payload.get('train'):
            ...
            # nn.save()
            nn.Origin_train(payload['trainArray'])
            nn.save()

        elif payload.get('predict'):
            try:
                # response = {"type":"test", "result":nn.predict(str(payload['image']))}
                # response = {"type": "test", "result": nn.MyPredict(str(payload['image']))}
                print("image = ", payload['image'])
                # res = nn.MyPredict(str(payload['image']))
                res = nn.MyPredict(payload['image'])
                print("res = ", res)
                response = {"type": "test", "result": str(res)}
            except:
                response_code = 500
        else:
            response_code = 400
        ...
        s.end_headers()
        if response:
            s.wfile.write(json.dumps(response).encode())  # 修改后
